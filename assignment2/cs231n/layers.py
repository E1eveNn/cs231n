...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    ###########################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    # x:(4,3) ,mean(4,1),var(4,1),normalized_x:(4,3),gamma:(3,),beta:(3,),out:(4,3)
    per_sample_mean=np.mean(x,axis=1,keepdims=True)
    per_sample_var=np.var(x,axis=1,keepdims=True)
    xmu=x-per_sample_mean
    sqrtvar=np.sqrt(per_sample_var+eps)
    normalized_x=xmu/sqrtvar
    out=gamma*normalized_x+beta
    cache=(xmu,sqrtvar,gamma,normalized_x)
...
def layernorm_backward(dout, cache):
    ...
    ###########################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    # dout:(4,3),normalzied_x:(4,3),gamma(3,),beta:(3,)
    xmu,sqrtvar,gamma,normalized_x=cache
    std_inv=1./sqrtvar
    N = np.prod(normalized_x.shape[1:])
    dgamma=np.sum(dout*normalized_x,axis=0)
    dbeta=np.sum(dout,axis=0)
    dnormalized_x=dout*gamma
    dvar = (-0.5) * np.sum(dnormalized_x * xmu,axis=-1,keepdims=True) * (std_inv ** 3)
    dmean = (-1.0) * np.sum(dnormalized_x * std_inv,axis=-1,keepdims=True)- 2.0 * dvar * np.mean(xmu,axis=-1,keepdims=True)
    dx = dnormalized_x * std_inv + (2. / N) * dvar * xmu    + (1. / N) * dmean
